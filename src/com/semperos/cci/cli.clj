...
;; be found in the file epl.html at the root of this distribution. By using this
...
This is a surgical copy-and-edit of clojure.tools.cli, removing parts that use
clojure.pprint/cl-format, to avoid features not currently supported by GraalVM."}
    com.semperos.cci.cli
...
#_(defn ^{:added "0.3.0"} format-lines
...
(defn ^{:added "0.3.0"} parse-opts
  ...
  (let [{:keys [in-order no-defaults strict summary-fn]} (apply hash-map options)
    ...
    {:options opts
     :arguments rest-args
     :summary (summary-fn #_(or summary-fn summarize) specs)
